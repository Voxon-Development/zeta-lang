...
fn run_compiler<'a>(
    ...
    for module_with_arena in stdlib_modules_result {
        if module_with_arena.parser_diagnostics.has_errors() {
            for error in &module_with_arena.parser_diagnostics.errors {
                error_reporter
                    .add_parser_error(StrId::from(arc.intern(error.to_string().as_str())), None);
            }
        }
    ...
    let out_obj: PathBuf = backend
        .finish(&out_dir)
        .map_err(CompilerError::FinishError)?;
    if !emit_obj {
